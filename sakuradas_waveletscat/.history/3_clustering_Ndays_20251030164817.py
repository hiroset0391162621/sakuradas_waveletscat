...
if __name__ == "__main__":
    ...
    # 複数日のデータを読み込むための設定
    # ==========================================
    start_date = datetime.datetime(2025, 5, 5, 0, 0, 0)  # 開始日時
    ...
    threshold_amplitude = 1e-10  # threshold for filtering out near-zero data
    ...
    threshold_amplitude = 1e-12
    valid_mask_plot = max_abs_per_sample_plot > threshold_amplitude
    
    print(f"Filtering for plot: {np.sum(~valid_mask_plot)} samples removed")
    print(f"Remaining samples for plot: {np.sum(valid_mask_plot)} / {len(valid_mask_plot)}")
