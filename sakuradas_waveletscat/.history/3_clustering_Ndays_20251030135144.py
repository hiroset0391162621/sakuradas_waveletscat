...
if __name__ == "__main__":
    ...
    cluster_and_plot_dendrogram(ustation_plot, copy.deepcopy(Z), threshold, default_color='black', pooling='max', savefig=True, output_dir=output_dir, filename_suffix=date_range_str)
    ...
    if len(stream_scat) == 0:
        ...
        try:
            ...
            for cid in cluster_ids:
                ...
                if hit_idx.size == 0:
                    continue
                # choose up to 10 examples (earliest in time)
                # sort chosen indices by time
                ...
                for k, idx_k in enumerate(chosen_idx):
                    ...
                    try:
                        ...
                        seg_f.filter('highpass', freq=0.5, corners=2, zerophase=True)
                    ...
                    axk.plot(x, y_norm, color='black', lw=0.6)
                ...
                fig.suptitle(f"{ustation} cluster {cid} examples (N={np.sum(predictions==cid)})\nHigh-pass 0.5 Hz, normalized", fontsize=12)
