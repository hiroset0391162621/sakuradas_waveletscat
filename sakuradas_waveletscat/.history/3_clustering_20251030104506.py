...
if __name__ == "__main__":
    ...
    # matplotlib形式の時刻をdatetime形式に変換
    times_matplotlib = trace.times("matplotlib")
    times_datetime = np.array([mdates.num2date(t) for t in times_matplotlib])
    
    ax1 = plt.subplot(211)
    ax1.plot(times_datetime, stream_scat[0].data/np.nanmax(np.abs(stream_scat[0].data)), color='black', lw=0.5)
