...
def plot_scattcoef_imshow(ustation, trace_x, trace_y):
    ...
    ax.xaxis.set_minor_locator(mdates.HourLocator(byhour=range(0,24), interval=1))
    ...
    ### major ticksを3時間ごと， minor ticksを1時間ごとに設定
    ax0.xaxis.set_major_locator(mdates.HourLocator(byhour=range(0,24), interval=3))
    ax0.xaxis.set_minor_locator(mdates.HourLocator(byhour=range(0,24), interval=1))

    plt.suptitle(ustation, fontsize=14, x=pos.x0+0.5*pos.width)

    print("save scattering figure", f"Figure/{fiber}/{hdf5_starttime_jst.strftime('%Y')}/{hdf5_starttime_jst.strftime('%m')}/{hdf5_starttime_jst.strftime('%d')}/{hdf5_starttime_jst.strftime('%H')}/scattering_coefficients_tchange_{ustation}_{hdf5_starttime_jst.strftime('%Y%m%d%H%M')}_{Nseconds}.png")

    plt.savefig(f"Figure/{fiber}/{hdf5_starttime_jst.strftime('%Y')}/{hdf5_starttime_jst.strftime('%m')}/{hdf5_starttime_jst.strftime('%d')}/{hdf5_starttime_jst.strftime('%H')}/scattering_coefficients_tchange_{ustation}_{hdf5_starttime_jst.strftime('%Y%m%d%H%M')}_{Nseconds}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    for ch_idx, target_channel in enumerate(used_channel_list):
        ...
        print('save scattering coefficients', "example/scattering_coefficients"+hdf5_starttime_jst.strftime("%Y%m%d%H%M")+"_"+str(Nseconds)+".npz")
        ...
        ### major ticksを3時間ごと， minor ticksを1時間ごとに設定
        ax[0].xaxis.set_major_locator(mdates.HourLocator(byhour=range(0,24), interval=3))
        ax[0].xaxis.set_minor_locator(mdates.HourLocator(byhour=range(0,24), interval=1))
        ax[1].xaxis.set_major_locator(mdates.HourLocator(byhour=range(0,24), interval=3))
        ax[1].xaxis.set_minor_locator(mdates.HourLocator(byhour=range(0,24), interval=1))


        # Show
        plt.suptitle(stream_scat[0].stats.network.lower()+stream_scat[0].stats.station+" "+stream_scat[0].stats.starttime.strftime("%Y-%m-%d %H:%M:%S")+"-"+stream_scat[0].stats.endtime.strftime("%Y-%m-%d %H:%M:%S"), fontsize=12)
        
        os.makedirs(f"Figure/{fiber}/{hdf5_starttime_jst.strftime('%Y')}/{hdf5_starttime_jst.strftime('%m')}/{hdf5_starttime_jst.strftime('%d')}/{hdf5_starttime_jst.strftime('%H')}", exist_ok=True)

        print('save psd figure', f"Figure/{fiber}/{hdf5_starttime_jst.strftime('%Y')}/{hdf5_starttime_jst.strftime('%m')}/{hdf5_starttime_jst.strftime('%d')}/{hdf5_starttime_jst.strftime('%H')}/psd_{ustation}_{hdf5_starttime_jst.strftime('%Y%m%d%H%M')}_{Nseconds}.png")

        plt.savefig(f"Figure/{fiber}/{hdf5_starttime_jst.strftime('%Y')}/{hdf5_starttime_jst.strftime('%m')}/{hdf5_starttime_jst.strftime('%d')}/{hdf5_starttime_jst.strftime('%H')}/psd_{ustation}_{hdf5_starttime_jst.strftime('%Y%m%d%H%M')}_{Nseconds}.png", dpi=300, bbox_inches="tight")
        plt.close()
