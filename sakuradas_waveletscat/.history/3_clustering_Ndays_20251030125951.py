...
if __name__ == "__main__":
    ...
    if len(stream_scat) == 0:
        ...
        for cluster_id in unique_predictions:
            ...
            if cluster_id<=7:
                ax2.plot(times_sorted, cum_counts / Nevents, label=f'Cluster {cluster_id} ({Nevents})', linewidth=1.5)
            if cluster_id>7:
                ax2.plot(times_sorted, cum_counts / Nevents, label=f'Cluster {cluster_id} ({Nevents})', linewidth=1.5, ls='--')
